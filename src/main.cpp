...
#if defined(NDEBUG)
# error "lacubanada cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcb798ad535155bd900bf7998dad8253e6688d7e1d738b11be496bb83b43a1462");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // lacubanada: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "lacubanada Signed Message:\n";
...
{
    // lacubanada: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // lacubanada: 420k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // lacubanada: 3 days
static const int64 nTargetSpacing = 2 * 60; // lacubanada: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // lacubanada: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // lacubanada: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9ab4f6842f79ea1d5339bb499534d05f59b66dbe5a8a7e9ac97bc9edef2dea98");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Pandemic era ends March 18 2021";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0461a835ee7c5db4755532c4d0243b7632e9ac7ead2244b7f1f9ffd7bc16b627f5c7e4a3c5ef54ce5be3a2acbf46d1ac9b2bd7feb9ac9df8a1b5b32be5b49e1d20") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1616110378;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087249344;

        if (fTestNet)
        {
            block.nTime    = 1616110315;
            block.nNonce   = 385390336;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xfad19c3c1ffe4c130df652c0f88d996c08dc9303466e29d76da203379d03eb1f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc0, 0xb5, 0xda }; // lacubanada: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("lacubanadaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("lacubanadaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static lacubanadaMiner(CWallet *pwallet)
{
    printf("lacubanadaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lacubanada-miner");
    ...
    try { loop {
        ...
        printf("Running lacubanadaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("lacubanadaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&lacubanadaMiner, pwallet));
}
